...
describe 'check_boxes input' do
  ...
  describe 'for a has_many association' do
    ...
    it_should_have_a_nested_div_with_class('controls')
    ...
    it 'should generate a \'legend\' containing a label with text for the input' do
      output_buffer.should have_tag('form fieldset.control-group label')
      output_buffer.should have_tag('form fieldset.control-group label', /Posts/)
    end

    it 'should not link the \'legend\' label to any input' do
      output_buffer.should_not have_tag('form fieldset.control-group > label[@for]')
    end

    it 'should generate a label for each choice' do
      output_buffer.should have_tag('form fieldset.control-group div.controls label.checkbox', :count => ::Post.all.size)
    end
    ...
    it 'should have one option with a "checked" attribute' do
      output_buffer.should have_tag('form input[@checked]', :count => 1)
    end
    ...
    it 'should not render hidden inputs inside the ol' do
      output_buffer.should_not have_tag("form div div input[@type='hidden']")
    end

    it 'should render one hidden input for each choice outside the ol' do
      output_buffer.should have_tag("form fieldset.control-group > input[@type='hidden']", :count => 1)
    end
    ...
    describe "each choice" do
      ...
      it 'should contain a label for the radio input with a nested input and label text' do
        ::Post.all.each do |post|
          output_buffer.should have_tag('form fieldset.control-group div.controls label', /#{post.to_label}/)
          output_buffer.should have_tag("form fieldset.control-group div.controls label[@for='author_post_ids_#{post.id}']")
        end
      end

      it 'should have a checkbox input but no hidden field for each post' do
        ::Post.all.each do |post|
          output_buffer.should have_tag("form fieldset.control-group div.controls label input#author_post_ids_#{post.id}")
          output_buffer.should have_tag("form fieldset.control-group div.controls label input[@name='author[post_ids][]']", :count => 1)
        end
      end

      it 'should have a hidden field with an empty array value for the collection to allow clearing of all checkboxes' do
        output_buffer.should have_tag("form fieldset.control-group > input[@type=hidden][@name='author[post_ids][]'][@value='']", :count => 1)
      end

      it 'the hidden field with an empty array value should be followed by the div.controls and the label' do
        output_buffer.should have_tag("form fieldset.control-group > input[@type=hidden][@name='author[post_ids][]'][@value=''] + div.controls label", :count => 1)
      end

      it 'should not have a hidden field with an empty string value for the collection' do
        output_buffer.should_not have_tag("form fieldset.control-group > input[@type=hidden][@name='author[post_ids]'][@value='']")
      end
      ...
      it 'should have a checkbox and a hidden field for each post with :hidden_field => true' do
        ...
        ::Post.all.each do |post|
          output_buffer.should have_tag("form fieldset.control-group div.controls label input#author_post_ids_#{post.id}")
          output_buffer.should have_tag("form fieldset.control-group div.controls label input[@name='author[post_ids][]']", :count => 2)
        end
      ...
      it "should mark input as checked if it's the the existing choice" do
        ...
        output_buffer.should have_tag("form fieldset.control-group div.controls label input[@checked='checked']")
      end
    end
    ...
    describe 'and no object is given' do
      ...
      it 'shold generate an li tag for each item in the collection' do
        output_buffer.should have_tag('form fieldset.control-group div.controls input[@type=checkbox]', :count => ::Author.all.size)
      end

      it 'should generate labels for each item' do
        ::Author.all.each do |author|
          output_buffer.should have_tag('form fieldset.control-group div.controls label', /#{author.to_label}/)
          output_buffer.should have_tag("form fieldset.control-group div.controls label[@for='project_author_id_#{author.id}']")
        end
      end

      it 'should generate inputs for each item' do
        ::Author.all.each do |author|
          output_buffer.should have_tag("form fieldset.control-group div.controls label input#project_author_id_#{author.id}")
          output_buffer.should have_tag("form fieldset.control-group div.controls label input[@type='checkbox']")
          output_buffer.should have_tag("form fieldset.control-group div.controls label input[@value='#{author.id}']")
          output_buffer.should have_tag("form fieldset.control-group div.controls label input[@name='project[author_id][]']")
        end
      end
      ...
      it 'should html escape the label string' do
        ...
        output_buffer.should have_tag('form fieldset.control-group div.controls label') do |label|
    ...
    describe 'when :hidden_fields is set to false' do
      ...
      it 'should have a checkbox input for each post' do
        ::Post.all.each do |post|
          output_buffer.should have_tag("form fieldset.control-group div.controls label input#author_post_ids_#{post.id}")
          output_buffer.should have_tag("form fieldset.control-group div.controls label input[@name='author[post_ids][]']", :count => ::Post.all.length)
        end
      end
      ...
      it 'should not generate empty hidden inputs' do
        output_buffer.should_not have_tag("form fieldset.control-group div.controls ul li label input[@type='hidden'][@value='']", :count => ::Post.all.length)
      end
    end
    ...
    describe 'when :disabled is set' do
      ...
      describe "no disabled items" do
        ...
        it 'should not have any disabled item(s)' do
          output_buffer.should_not have_tag("form fieldset.control-group div.controls ul li label input[@disabled='disabled']")
        end
      end
      ...
      describe "single disabled item" do
        ...
        it "should have one item disabled; the specified one" do
          output_buffer.should have_tag("form fieldset.control-group div.controls label input[@disabled='disabled']", :count => 1)
          output_buffer.should have_tag("form fieldset.control-group div.controls label[@for='post_author_ids_#{@fred.id}']", /fred/i)
          output_buffer.should have_tag("form fieldset.control-group div.controls label input[@disabled='disabled'][@value='#{@fred.id}']")
        end
      end
      ...
      describe "multiple disabled items" do
        ...
        it "should have multiple items disabled; the specified ones" do
          output_buffer.should have_tag("form fieldset.control-group div.controls label input[@disabled='disabled']", :count => 2)
          output_buffer.should have_tag("form fieldset.control-group div.controls label[@for='post_author_ids_#{@bob.id}']", /bob/i)
          output_buffer.should have_tag("form fieldset.control-group div.controls label input[@disabled='disabled'][@value='#{@bob.id}']")
          output_buffer.should have_tag("form fieldset.control-group div.controls label[@for='post_author_ids_#{@fred.id}']", /fred/i)
          output_buffer.should have_tag("form fieldset.control-group div.controls label input[@disabled='disabled'][@value='#{@fred.id}']")
        end
      end
    ...
    describe "with i18n of the legend label" do
      ...
      it "should do foo" do
        output_buffer.should have_tag("fieldset.control-group > label", /Translated/)
      end
    ...
    describe "when :label option is set" do
      ...
      it "should output the correct label title" do
        output_buffer.should have_tag("fieldset.control-group > label", /The authors/)
      end
    end
    ...
    describe "when :label option is false" do
      ...
      it "should output an empty \'legend\'" do
        output_buffer.should have_tag("fieldset.control-group > label", "")
      end
    ...
    describe "when :required option is true" do
      ...
      it "should output the correct label title" do
        output_buffer.should have_tag("fieldset.control-group > label abbr")
      end
    end
  ...
  describe 'for an association when a :collection is provided' do
    describe 'it should use the specified :member_value option' do
      ...
      it 'to set the right input value' do
        ...
        @new_post.author.should_receive(:custom_value).once
        concat(semantic_form_for(@new_post) do |builder|
  ...
  describe 'when :collection is provided as an array of arrays' do
    ...
    it 'should check the correct checkboxes' do
      output_buffer.should have_tag("form fieldset.control-group div.controls label input[@value='fiction'][@checked='checked']")
      output_buffer.should have_tag("form fieldset.control-group div.controls label input[@value='biography'][@checked='checked']")
    end
  end
  ...
  describe "when namespace is provided" do
    ...
    it "should have a label for #context2_author_post_ids_19" do
      output_buffer.should have_tag("form label[@for='context2_author_post_ids_19']")
    end
  ...
  describe "when collection is an array" do
    ...
    it "should use array items for labels and values" do
      @_collection.each do |post|
        output_buffer.should have_tag('form fieldset.control-group div.controls label', /#{post.first}/)
        output_buffer.should have_tag("form fieldset.control-group div.controls label[@for='author_post_ids_#{post.last}']")
      end
    end
